...
- **手機端進階設定** — 用手機掃描 QR 碼設定允許的應用和觀看時間規則（需同一 Wi-Fi）
- **限制應用管控** — 臨時解鎖期間阻止未授權應用啟動，防止孩子繞過限制
- **觀看時間排程** — 設定每週觀看時段，到時自動開啟臨時解鎖，支援會話時長和每日次數限制
- **使用說明 QR 碼** — 引導頁"?"按鈕一鍵查看使用說明
- **權限設定引導** — 引導使用者完成權限設定，確保鎖定功能正常運行

## 使用方法

1. 在 Android TV 或電視盒子上安裝 APK
   - 需要安裝協助？使用 [安裝助手](https://tv.asdasd.vip/tools) 或讓 [ChatGPT](https://chatgpt.com/?q=%E6%99%BA%E8%83%BD%E7%94%B5%E8%A7%86%E5%A6%82%E4%BD%95%E5%AE%89%E8%A3%9D%20APK) 指導你
2. 開啟應用，按照引導授予所需權限
